...
function startExecution() {
    ...
    jQuery('#welcomeMsg').html(welcomeMsg);
    ...
    userIcon = baseUrl+"../../images/user-icon/young-lady.png";
...
function chatbot() {
    ...
    previewMessageBackGroundColor = "F6B20E";
    prevWindowHeight = true;
    // jQuery('#syraChatInnerDiv div').remove();
    var control_h = '<!DOCTYPE HTML>' +
    ...
    var control_b = '' +
        ...
        '<span id="badge_text" class="badge_taxbot_syra" style ="background:#'+previewMessageBackGroundColor+';"></span>' +
        '<img class="clickableChatBotIcon" src="' + syraIconWithInChatBotLightBase + '" height="100%" width="100%" style="margin-left: 5px;">' +
        '</a>' +
        ...
        '</li>' +
        '<li>' +
        '<a href="http://cloudhiti.ai/" target="_blank" id="a_mail">' +
        '<span>' +
        ...
        '<a href="https://syra.ai/" target="_blank"><p>AI Chatbots for Business Good. Get Yours Today!</p></a>' +
        '</div>' +
        '</div>' +
        '</div>' +
    ...
    jQuery('#badge_text').html(previewMessage);
    jQuery('#chatstart').fadeToggle();
    chatbotHeightCalc();
    startExecution();
    if (document.cookie.match(new RegExp('(^| )shopify_access_token=([^;]+)')) != null) {
        changeDefaultIconForShopify('icon-40.png');

        jQuery("#questionNo1").attr("name", "Help me find items that I can buy.");
        jQuery("#questionNo1").text("Find Items");

        /*jQuery("#questionNo2").attr("name", "What are the items that are on sale today?");
        jQuery("#questionNo2").text("Today's Promotions");*/

        jQuery("#questionNo3").attr("name", "Help me locate your stores.");
        jQuery("#questionNo3").text("Locate Stores");

        jQuery("#questionNo4").attr("name", "Get the latest status of my order.");
        jQuery("#questionNo4").text("Order Status");

        jQuery("#questionNo5").attr("name", "I want to send a message to you.");
        jQuery("#questionNo5").text("Send a Message");
    }

}
var nAgtSyra = navigator.userAgent;
function chatbotHeightCalc() {
    // console.log("------------" + document.documentElement.clientHeight);
    if(nAgtSyra.indexOf("Chrome")!=-1){
        if(prevWindowHeight == true){
            jQuery("#syraChatDiv_syra").height(jQuery(window).height() - 391);
        }
        else{
            jQuery("#syraChatDiv_syra").height(jQuery(window).height() - 250); //it works on google chrome
        }
    }
    else if(nAgtSyra.indexOf("Safari")!=-1){
        if(prevWindowHeight == true){
            jQuery("#syraChatDiv_syra").height(jQuery(window).height() - 391);
        }
        else{
            jQuery("#syraChatDiv_syra").height(jQuery(window).height() - 370);
        }
    }
    else{
...
function a_thumbsdownClick() {
    ...
    var incomingMsg = '<div class="incoming_msg_syra incoming_msg_padding_syra" style="padding-top:25px">' +
        ...
        '</div>' +
        '<div class="incom_mess_text_syra" style="background-color:' + botResponseBackgroundColor + '">' +
        '<p> We are sorry to note that you did not have a great experience. You also email us at <b>feedback@cloudhiti.ai</b> with your comments.</p>' +
        '</div>' +
        '<div class="timer_syra timer1_syra">' +
        '<span>a second ago</span>' +
        '</div>' +
...
function changeDefaultIconForShopify(rate_value) {
    var image_url = "images/chatbot-icons/" + rate_value;
